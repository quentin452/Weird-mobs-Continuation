...
public class EntityStickManBaby extends EntityAnimal {
   ...
   public void onLivingUpdate() {
      ...
      if (this.getLifeTime() >= this.life) {
         ...
         for(int i = 0; i < this.rand.nextInt(50) + 100; ++i) {
            this.worldObj.spawnParticle("smoke", this.posX + this.rand.nextGaussian() * 0.3D, this.posY + this.rand.nextGaussian() * 0.3D, this.posZ + this.rand.nextGaussian() * 0.3D, 0.0D, 0.0D, 0.0D);
         }
   ...
   public boolean interact(EntityPlayer player) {
      ...
      if (!this.isSpeedUpItem(item)) {
         ...
         for(int i = 0; i < 10; ++i) {
            this.worldObj.spawnParticle("happyVillager", this.posX + this.rand.nextGaussian() * 0.1D, this.posY + this.rand.nextGaussian() * 0.1D + (double)(this.height / 2.0F), this.posZ + this.rand.nextGaussian() * 0.1D, 0.1D, 0.0D, 0.1D);
         }
