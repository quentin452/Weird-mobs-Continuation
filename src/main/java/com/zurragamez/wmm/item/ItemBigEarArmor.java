...
public class ItemBigEarArmor extends ItemArmor {
    ...
    public String getArmorTexture(ItemStack item, Entity e, int slot, String type) {
        if (item.getItem() != WeirdMobsMod.bigEarArmorHelmet && item.getItem() != WeirdMobsMod.bigEarArmorChestplate
            && item.getItem() != WeirdMobsMod.bigEarArmorBoots) {
            return item.getItem() == WeirdMobsMod.bigEarArmorLeggings
                ? "WeirdMobsMod:textures/models/armor/BigEarArmor_2.png"
                : null;
        } else {
